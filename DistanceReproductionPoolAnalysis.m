...
for i = m
    ...
    if ~isnan(viewDistance)
        ...
        fixPos = double(d.fix_y{i}(1,2));
        ds1{i} = viewDistance*( tand(fixPos) - tand(fixPos-ds1{i}) );
        ds2{i} = viewDistance*( tand(fixPos) - tand(fixPos-ds2{i}) );
        ds{i} = viewDistance*( tand(fixPos) - tand(fixPos-ds{i}) );
        dp{i} = viewDistance*( tand(fixPos) - tand(fixPos-dp{i}) );
    end
...
for fits = 1:length(fittype)
    ...
    switch fittype{fits}
        ...
        case 'EKF'
            switch Fit.method
                case 'quad'
                    ...
                    dt = Fit.dx;
                    LapseSupport = [MinMaxDp(1) max(dss)+MinMaxDp(2)];
                    [WM(:,fits), WP(:,fits,1), B(:,fits), lapse(:,fits), SIGP(:,fits),...
        ...
        case 'BLS_wm1wm2'
            switch Fit.method
                case 'quad'
                    % Fit the BLS_wm1wm2 model with bias and lapses to the data
                    disp('Fitting BLS_wm1wm2 with bias and lapses model to the data using Simpsons quadrature...')
                    init = Fit.init;
                    if isstr(init)
                        switch init
                            case 'estb'
                                init = [0.1 0.06 0.2 NaN 0 0];      % Default initial search but with estimate of baseline
                                estb(i) = nanmean(dpIn{i}) - nanmean(dsIn{i});
                                init(4) = estb(i);
                            case 'default'
                                init = [0.1 0.06 0.2 0 0 0];
                        end
                    end
                    dt = Fit.dx;
                    LapseSupport = [MinMaxDp(1) max(dss)+MinMaxDp(2)];
